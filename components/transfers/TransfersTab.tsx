  ...
  return (
    <PageWrap title="Transferlar" action={<button className="btn-primary" onClick={() => setShowModal(true)}>+ Yangi so'rov</button>}>
      {showModal && <Modal onClose={() => setShowModal(false)}>
        ...
        <div className="modal-actions" style={{ display: "flex", gap: 10 }}><button className="btn-ghost" onClick={() => setShowModal(false)} style={{ flex: 1 }}>{t.cancel}</button><button className="btn-primary" onClick={submit} disabled={loading} style={{ flex: 2 }}>{loading ? "Yuborilmoqda..." : `📤 ${t.send}`}</button></div>
      </Modal>}
      ...
      {action && <Modal onClose={() => !loading && setAction(null)}>
        ...
        <div className="modal-actions" style={{ display: "flex", gap: 10 }}><button className="btn-ghost" disabled={loading} onClick={() => setAction(null)} style={{ flex: 1 }}>Bekor</button><button className="btn-primary" disabled={loading} onClick={submitAction} style={{ flex: 2 }}>{loading ? "Saqlanmoqda..." : "Jo'natish va skladga qo'shish"}</button></div>
      </Modal>}
