...
import { uploadShopSalesExcelApi } from "@/lib/api";
...
function unwrapList(data: any): any[] {
  const value =
    data?.data?.results ?? data?.data?.items ?? data?.data?.shopSales ?? data?.data?.shop_sales ??
    data?.results ?? data?.items ?? data?.shopSales ?? data?.shop_sales ?? data?.data ?? data;
...
function normalizeSaleItem(item: any) {
  return {
    ...
    productId: String(item?.productId ?? item?.product_id ?? item?.product?.id ?? item?.product ?? ""),
    productName: item?.productName ?? item?.product_name ?? item?.product?.name ?? item?.sourceName ?? item?.source_name ?? "",
    quantity: n(item?.quantity ?? item?.qty),
    salesAmount: n(item?.salesAmount ?? item?.sales_amount ?? item?.total_sales ?? item?.sales ?? item?.amount),
    costAmount: n(item?.costAmount ?? item?.cost_amount ?? item?.total_cost ?? item?.cost),
    profitAmount: n(item?.profitAmount ?? item?.profit_amount ?? item?.total_profit ?? item?.profit),
    averagePrice: n(item?.averagePrice ?? item?.average_price ?? item?.price),
    stockBefore: n(item?.stockBefore ?? item?.stock_before),
    stockAfter: n(item?.stockAfter ?? item?.stock_after),
    shortage: n(item?.shortage),
  };
}
...
function normalizeShopSale(raw: any): ShopSaleImport {
  ...
  return {
    ...
    totalCost: n(raw?.totalCost ?? raw?.total_cost) || items.reduce((s: number, row: any) => s + row.costAmount, 0),
    totalProfit: n(raw?.totalProfit ?? raw?.total_profit) || items.reduce((s: number, row: any) => s + row.profitAmount, 0),
    shortageCount: n(raw?.shortageCount ?? raw?.shortage_count),
    skippedRows: raw?.skippedRows ?? raw?.skipped_rows ?? [],
...
export function ShopSalesTab({ shopStock, shopSales, user, branches, selectedBranchSlug = "", fetchAll, showToast, setTab }: Props) {
  ...
  const loadShopSales = async () => {
    ...
    try {
      ...
      console.warn("[shop-sales] direct GET failed", error);
    } finally {
  ...
  const daily = Object.values(imports.reduce((map: Record<string, { date: string; sales: number; profit: number; quantity: number }>, item) => {
    if (!item.saleDate) return map;
    const cur = map[item.saleDate] || { date: item.saleDate, sales: 0, profit: 0, quantity: 0 };
    cur.sales += n(item.totalSales);
    cur.profit += n(item.totalProfit);
    cur.quantity += n(item.totalQuantity);
    map[item.saleDate] = cur;
    return map;
  }, {})).sort((a, b) => b.date.localeCompare(a.date));

  const periodStats = Object.values(imports.reduce((map: Record<string, { key: string; sales: number; profit: number; quantity: number }>, item) => {
    const key = period === "daily" ? item.saleDate : period === "monthly" ? item.saleDate.slice(0, 7) : item.saleDate.slice(0, 4);
    if (!key) return map;
    const cur = map[key] || { key, sales: 0, profit: 0, quantity: 0 };
    cur.sales += n(item.totalSales);
    cur.profit += n(item.totalProfit);
    cur.quantity += n(item.totalQuantity);
    map[key] = cur;
  ...
  const productStats = Object.values(filteredImports.flatMap((i) => i.items || []).reduce((map: Record<string, any>, item: any) => {
    const id = item.productId || item.barcode || item.productName;
    const cur = map[id] || { id, name: item.productName || item.sourceName, barcode: item.barcode, quantity: 0, sales: 0, profit: 0 };
    cur.quantity += n(item.quantity);
    cur.sales += n(item.salesAmount);
    cur.profit += n(item.profitAmount);
    map[id] = cur;
    return map;
  }, {})).sort((a: any, b: any) => b.sales - a.sales);

  const refreshAfterUpload = async (message: string) => {
    showToast(message);
    await fetchAll();
  ...
  return (
    <PageWrap
      ...
      action={canImportExcel ? <label className="btn-primary" style={{ display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer", position: "relative", overflow: "hidden" }}><Upload size={17} />{reading ? "Yuborilmoqda..." : "Excel import"}<input aria-label="Excel faylni tanlash" data-testid="shop-sales-excel-input" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(e) => { void readFile(e.target.files?.[0]); e.target.value = ""; }} disabled={reading} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0, cursor: "pointer" }} /></label> : undefined}
    >
      <div className="sales-filter-row">
        ...
        <div style={{ fontSize: 12, color: "var(--app-muted)", alignSelf: "end", paddingBottom: 10 }}>{loadingSales ? "Yuklanmoqda..." : `${filteredImports.length} ta import`}</div>
      </div>
