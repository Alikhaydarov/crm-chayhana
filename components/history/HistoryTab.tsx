...
import { CalendarDays, ChevronLeft, ChevronRight, CircleDollarSign, Search, ShoppingCart } from "lucide-react";
import { PageWrap } from "@/components/ui";
import { fmtM } from "@/lib/utils";
import type { Company, CompanyPayment, Order } from "@/types/domain";

type Filter = "all" | "orders" | "payments";
type Event = { id: string; type: "order" | "payment"; date: string; company: string; amount: number; detail: string; orderId: string };
...
export function HistoryTab({ orders, payments, companies, lang }: { orders: Order[]; payments: CompanyPayment[]; companies: Company[]; lang: string }) {
  const today = dayKey();
  ...
  const counts = useMemo(() => events.reduce<Record<string, { orders: number; payments: number }>>((acc, event) => {
    acc[event.date] ||= { orders: 0, payments: 0 };
    acc[event.date][event.type === "order" ? "orders" : "payments"] += 1;
    return acc;
  }, {}), [events]);
  const visible = events.filter((event) => event.date === selectedDay && (filter === "all" || `${event.type}s` === filter) && `${event.company} ${event.orderId}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase()));
  ...
  return <PageWrap title="Tarix" sub="Orderlar va firmalarga qilingan to'lovlar">
    ...
    <div className="history-layout">
      <section className="history-calendar">
        ...
        <div className="history-days">{days.map((date) => { const key = dayKey(date.toISOString()); const count = counts[key]; return <button key={key} className={`${date.getMonth() !== month.getMonth() ? "outside " : ""}${key === today ? "today " : ""}${key === selectedDay ? "selected" : ""}`} onClick={() => setSelectedDay(key)}><span>{date.getDate()}</span>{count && <div><i className="order-dot" />{count.payments > 0 && <i className="payment-dot" />}</div>}</button>; })}</div>
      ...
      <section className="history-events">
        <div className="history-toolbar"><div className="history-filters">{(["all", "orders", "payments"] as Filter[]).map(value => <button key={value} className={filter === value ? "active" : ""} onClick={() => setFilter(value)}>{value === "all" ? "Barchasi" : value === "orders" ? "Orderlar" : "To'lovlar"}</button>)}</div><label><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Firma yoki order ID" /></label></div>
        ...
        <div className="history-event-list">{visible.map(event => <article key={event.id}><div className={`history-event-icon ${event.type}`} >{event.type === "order" ? <ShoppingCart size={17} /> : <CircleDollarSign size={17} />}</div><div className="history-event-main"><strong>{event.company}</strong><span>{event.type === "order" ? "Yangi order" : "Firma to'lovi"} · #{event.orderId.slice(-8)}</span><small>{event.detail}</small></div><div className={`history-event-amount ${event.type}`}><strong>{event.type === "payment" ? "+" : ""}{fmtM(event.amount)}</strong><span>{event.type === "order" ? "Order" : "To'lov"}</span></div></article>)}{!visible.length && <div className="history-empty"><CalendarDays size={28} /><strong>Bu kunda tarix yo'q</strong><span>Boshqa sana yoki filtrni tanlang</span></div>}</div>
      </section>
    </div>
  </PageWrap>;
}
