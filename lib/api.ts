...
import { branchForUser, branchTypeOf } from "@/lib/permissions";
...
function normalizeUser(data: any): AppUserInfo { const unwrapped = unwrap<any>(data); const value = unwrapped?.user ?? unwrapped; const branch = value?.branch_detail ?? value?.branch ?? {}; const rawAccountRole = value?.role ?? value?.accountRole; const accountRole = typeof rawAccountRole === "string" ? rawAccountRole.toLowerCase() : rawAccountRole; const branchId = value?.branch_id ?? value?.branchId ?? branch?.id ?? (typeof value?.branch === "number" || typeof value?.branch === "string" ? value.branch : undefined); const branchSlug = value?.branch_slug ?? value?.branchSlug ?? branch?.slug ?? branch?.warehouse?.branch_slug; const rawBranchType = value?.branch_type ?? value?.branchType ?? value?.type ?? branch?.branch_type ?? branch?.branchType ?? branch?.type; const branchType = branchTypeOf({ branch_type: rawBranchType }); const branchName = value?.branchName ?? value?.branch_name ?? branch?.name ?? ""; const isShopBranch = branchType === "shop" || (!branchType && /shop|dokon|do-kon|do'kon/i.test(`${branchSlug || ""} ${branchName}`)); const role = accountRole === "ceo" || accountRole === "super_admin" ? "superadmin" : isShopBranch ? "shop" : accountRole === "admin" ? "restaurant1" : accountRole ?? value?.branchCode ?? branchSlug; return { ...value, id: String(value?.id ?? value?.userId ?? value?.username ?? ""), name: value?.name || value?.fullName || [value?.first_name, value?.last_name].filter(Boolean).join(" ") || value?.username || value?.userId || "", role, accountRole, branchId, branchSlug, branchType: branchType || undefined, branchName: branchName || branchSlug || role || "", branchIcon: value?.branchIcon ?? value?.branch_icon ?? branch?.icon ?? (isShopBranch ? "🏪" : "🍽️") } as AppUserInfo; }
...
async function enrichUserBranch(user: AppUserInfo): Promise<AppUserInfo> { if (user.role === "superadmin") return user; let value: any = null; if (user.branchSlug) value = unwrap<any>(await optionalRequest<any>(`/branches/${encodeURIComponent(user.branchSlug)}/`, null)); if (!value) { const branches = unwrapList<any>(await optionalRequest<any>("/branches/?page_size=1000", [])); value = branches.find((branch) => (user.branchId != null && String(branch.id) === String(user.branchId)) || (user.branchSlug && branch.slug === user.branchSlug) || (user.branchName && branch.name === user.branchName)); } if (!value) return user; const branchType = branchTypeOf(value); return { ...user, role: branchType === "shop" ? "shop" : "restaurant1", branchId: value.id, branchSlug: value.slug ?? value.branch_slug ?? value.warehouse?.branch_slug ?? user.branchSlug, branchType: branchType || user.branchType, branchName: value.name || user.branchName, branchIcon: branchType === "shop" ? "🏪" : "🍽️" }; }
...
export async function getSnapshotApi(user: AppUserInfo) {
  const userBranch = branchForUser(user);
  try {
    ...
    if (user.role !== "superadmin") { snapshot.transfers = (snapshot.transfers || []).filter((transfer: any) => (transfer.toBranch ?? transfer.to_branch ?? transfer.branch_slug) === userBranch); if (user.role === "shop") { snapshot.companies = []; snapshot.orders = []; snapshot.companyPayments = []; snapshot.staff = (snapshot.staff || []).filter((member: any) => (member.branchSlug ?? member.branch_slug ?? member.branch) === userBranch); } else { snapshot.shopSales = []; snapshot.shopStock = {}; snapshot.staff = (snapshot.staff || []).filter((member: any) => (member.branchSlug ?? member.branch_slug ?? member.branch) === userBranch); } }
    return snapshot;
  } catch (snapshotError) {
    ...
    const [productsData, mainStockData, shopStockData, transfersData, reportsData, companiesData, ordersData, shopSalesData, branchesData, usersData] = await Promise.all([
      ...
      optionalRequest<any>(user.role === "superadmin" ? "/stock/main/" : `/stock/branches/${encodeURIComponent(userBranch)}/`, {}),
      user.role === "superadmin" || user.role === "shop" ? optionalRequest<any>(user.role === "superadmin" ? "/stock/branches/shop/" : `/stock/branches/${encodeURIComponent(userBranch)}/`, {}) : Promise.resolve({}),
    ...
    return { products: unwrapList<Product>(productsData), stock: normalizeStock(mainStockData), mainStock: normalizeStock(mainStockData), shopStock: normalizeStock(shopStockData), transfers: user.role === "superadmin" ? unwrapList<any>(transfersData) : unwrapList<any>(transfersData).filter(transfer => (transfer.toBranch ?? transfer.to_branch ?? transfer.branch_slug) === userBranch), reports: unwrap<any>(reportsData), companies, orders: unwrapList<any>(ordersData), companyPayments: paymentGroups.flatMap(group => unwrapList<any>(group)), shopSales: unwrapList<any>(shopSalesData), branches: unwrapList<any>(branchesData), accounts: unwrapList<any>(usersData).map(normalizeAccount) };
  }
}
