...
// If the role from the backend is already one of the four real roles, use
// it directly rather than routing it through the heuristic below (which
// exists for a different, legacy integration that sends accountRole values
// like "ceo"/"admin" and infers "shop" from the branch name). The direct
// match keeps a correct backend role safe from ever being second-guessed by
// that heuristic; every login through this codebase's own /auth/login
// already lands on this fast path today, so this changes no current
// behavior, just makes it explicit instead of accidental.
function normalizeUser(data: any): AppUserInfo { const unwrapped = unwrap<any>(data); const value = unwrapped?.user ?? unwrapped; const rawAccountRole = value?.role ?? value?.accountRole; const accountRole = typeof rawAccountRole === "string" ? rawAccountRole.toLowerCase() : rawAccountRole; const branchId = value?.branch_id ?? value?.branchId ?? value?.branch?.id ?? (typeof value?.branch === "number" || typeof value?.branch === "string" ? value.branch : undefined); const branchSlug = value?.branch_slug ?? value?.branchSlug ?? value?.branch?.slug; const branchType = value?.branch_type ?? value?.branchType ?? value?.branch?.branch_type; const branchName = value?.branchName ?? value?.branch_name ?? value?.branch?.name ?? ""; const isShopBranch = branchType === "shop" || /shop|dokon|do-kon|do'kon/i.test(`${branchSlug || ""} ${branchName}`); const validRoles = ["superadmin", "restaurant1", "restaurant2", "shop"]; const role = validRoles.includes(accountRole) ? accountRole : accountRole === "ceo" || accountRole === "super_admin" ? "superadmin" : isShopBranch ? "shop" : accountRole === "admin" ? "restaurant1" : accountRole ?? value?.branchCode ?? branchSlug; return { ...value, id: String(value?.id ?? value?.userId ?? value?.username ?? ""), name: value?.name || value?.fullName || [value?.first_name, value?.last_name].filter(Boolean).join(" ") || value?.username || value?.userId || "", role, accountRole, branchId, branchSlug: branchSlug || (isShopBranch ? "shop" : undefined), branchType, branchName: branchName || branchSlug || role || "", branchIcon: value?.branchIcon ?? value?.branch_icon ?? value?.branch?.icon ?? "" } as AppUserInfo; }
...
export async function createDamageRequestApi(data: { branch: string; productId: string; quantity: number; reason: string; image?: OrderReceipt }): Promise<ApiResult<{ data: DamageRequest }>> {
  try {
    ...
    if (data.image) {
      ...
      const upload = await fetch(signed.uploadUrl, { method: "PUT", headers: { "content-type": file.type, "x-upsert": "false" }, body: file }); if (!upload.ok) throw new Error("Brak rasmini Storage'ga yuklab bo'lmadi");
