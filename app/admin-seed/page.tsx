...
export default function AdminSeedPage(){
 ...
 async function api(path:string,opt:RequestInit={}){
  ...
  if(!r.ok||d?.success===false) throw new Error(d?.message||d?.detail||d?.errors?.detail||`API xatosi (${r.status})`); return d;
 }
 ...
 async function run(){
  ...
  try{
   ...
   for(let i=0;i<P.length;i++){
    ...
    if(!product){
     product=unwrap(await api("/products/",{method:"POST",body:JSON.stringify({id:`excel_${Date.now()}_${i}`,name,category:"excel-import",categoryName:"excel-import",unit:"dona",minStock:0,pricePerUnit:Number(sale||price||0),perBox:0,boxUnit:"",qrCode:String(barcode),supplierId:companyId})}));
     by.set(String(barcode),product); created++;
    }else exists++;
    const productId=String(product?.external_id??product?.externalId??product?.id??""); if(!productId) throw new Error(`${name} ID qaytmadi`);
    items.push({productId,quantity:Number(quantity||1),pricePerUnit:Number(price||sale||0)});
   }
   setS("Skladga kirim qilinmoqda..."); await api("/orders/",{method:"POST",body:JSON.stringify({companyId,items,note:"Excel test kirim 2026-05-30",payStatus:"unpaid",paidAmount:0,orderDate:"2026-05-30"})});
