...
type Product = { id:string; name:string; category:string; unit:string; minStock:number; pricePerUnit:number; perBox:number; boxUnit:string; qrCode?:string; supplierId?:string };
...
const I18N:Record<Lang,Record<string,string>> = {
  uz: {
    dashboard:"Dashboard", warehouse:"Sklad", transfers:"Transferlar", orders:"Order", products:"Mahsulotlar", suppliers:"Firmalar",
    logout:"Chiqish", language:"Til", uzbek:"O'zbek", korean:"Koreys",
    dark:"Tungi", light:"Kunduzgi", mode:"Rejim", loading:"Yuklanmoqda...",
    save:"Saqlash", cancel:"Bekor qilish", close:"Yopish", edit:"O'zgartirish", view:"Ko'rish",
    approve:"Tasdiqlash", reject:"Rad etish", sending:"Yuborilmoqda...",
    pending:"Kutilmoqda", approved:"Tasdiqlandi", rejected:"Rad etildi", partial:"Qisman",
    paid:"To'langan", unpaid:"To'lanmagan",
    lowStock:"Kam qoldi", medium:"O'rtacha", good:"Yaxshi",
    searchProduct:"Mahsulot qidirish...", noProductsFound:"Mahsulot topilmadi",
    addNewProduct:"Yangi mahsulot qo'shish", productName:"Mahsulot nomi",
    category:"Kategoriya", unit:"Birlik", minStock:"Min sklad", price:"Narx",
    qrCode:"QR / Shtrix kod", scan:"Skan", unitsPerBox:"Qutidagi soni", unitName:"Birlik nomi",
    firm:"Firma", selectFirm:"Firma tanlang", noFirmsYet:"Firma qo'shilmagan",
    addFirm:"+ Yangi firma", firmName:"Firma nomi", address:"Manzil", phone:"Telefon",
    addedDate:"Qo'shilgan sana", totalOrders:"Jami orderlar", debt:"Qarz", noDebt:"Yo'q",
    info:"Info", pay:"Pay", history:"Tarix",
    totalDebt:"Umumiy qarz", totalPaid:"To'langan", totalAmount:"Jami summa",
    payAmount:"To'lov summasi", payAll:"Hammasi", note:"Izoh", payNote:"Naqd, bank...",
    noOrders:"Order yo'q", noHistory:"To'lov tarixi yo'q",
    newOrder:"+ Yangi order", selectProduct:"Mahsulot tanlang", quantity:"Soni",
    payStatus:"To'lov holati", totalPrice:"Jami summa", savedToStock:"Skladga qo'shildi",
    newRequest:"+ Yangi so'rov", transferRequest:"Transfer so'rovi", selectBranch:"Filial",
    addProduct:"+ Mahsulot qo'shish", transferNote:"Eslatma", send:"Yuborish",
    transferDetails:"Transfer tafsilotlari", requestedBy:"So'ragan", approvedBy:"Tasdiqlagan",
    noTransfers:"Transfer yo'q", recentTransfers:"Oxirgi transferlar", viewAll:"Barchasi",
    stockValue:"Sklad qiymati", productTypes:"Mahsulot turlari", totalStaff:"Jami xodim",
    orderDebt:"Order qarzi", myStock:"Skladim", myRequests:"So'rovlarim", waiting:"Kutilmoqda",
    firms:"Firmalar", branchStatus:"Filiallar", editStock:"Miqdorni o'zgartirish",
    currentQty:"Hozirgi", newQty:"Yangi miqdor", stockUpdated:"Sklad yangilandi",
    invalidQty:"Noto'g'ri miqdor", total:"Jami", branch:"Filial", date:"Sana",
  },
  ko: {
    dashboard:"대시보드", warehouse:"창고", transfers:"이동", orders:"주문", products:"상품", suppliers:"업체",
    logout:"로그아웃", language:"언어", uzbek:"우즈벡어", korean:"한국어",
    dark:"다크", light:"라이트", mode:"모드", loading:"로딩 중...",
    save:"저장", cancel:"취소", close:"닫기", edit:"수정", view:"보기",
    approve:"승인", reject:"거절", sending:"전송 중...",
    pending:"대기", approved:"승인됨", rejected:"거절됨", partial:"부분",
    paid:"완료", unpaid:"미결제",
    lowStock:"부족", medium:"주의", good:"양호",
    searchProduct:"상품 검색...", noProductsFound:"상품 없음",
    addNewProduct:"새 상품 추가", productName:"상품명",
    category:"카테고리", unit:"단위", minStock:"최소 재고", price:"가격",
    qrCode:"QR 코드", scan:"스캔", unitsPerBox:"박스당 수량", unitName:"단위명",
    firm:"업체", selectFirm:"업체 선택", noFirmsYet:"업체 없음",
    addFirm:"+ 새 업체", firmName:"업체명", address:"주소", phone:"전화",
    addedDate:"등록일", totalOrders:"총 주문", debt:"미수금", noDebt:"없음",
    info:"정보", pay:"결제", history:"내역",
    totalDebt:"총 미수금", totalPaid:"결제됨", totalAmount:"총 금액",
    payAmount:"결제 금액", payAll:"전액", note:"메모", payNote:"현금, 은행...",
    noOrders:"주문 없음", noHistory:"결제 내역 없음",
    newOrder:"+ 새 주문", selectProduct:"상품 선택", quantity:"수량",
    payStatus:"결제 상태", totalPrice:"총 금액", savedToStock:"창고에 추가됨",
    newRequest:"+ 새 요청", transferRequest:"이동 요청", selectBranch:"지점",
    addProduct:"+ 상품 추가", transferNote:"메모", send:"전송",
    transferDetails:"이동 상세", requestedBy:"요청자", approvedBy:"승인자",
    noTransfers:"이동 없음", recentTransfers:"최근 이동", viewAll:"전체",
    stockValue:"재고 금액", productTypes:"상품 종류", totalStaff:"전체 직원",
    orderDebt:"주문 미수금", myStock:"내 재고", myRequests:"내 요청", waiting:"대기",
    firms:"업체", branchStatus:"지점 현황", editStock:"재고 수량 수정",
    currentQty:"현재", newQty:"새 수량", stockUpdated:"재고 수정됨",
    invalidQty:"수량 오류", total:"합계", branch:"지점", date:"날짜",
  },
};

const BNAME:Record<string,string> = { restaurant1:"Oshxona-1", restaurant2:"Oshxona-2", shop:"Do'kon", main:"Bosh Sklad" };
const BICON:Record<string,string> = { restaurant1:"🍽️", restaurant2:"🍜", shop:"🏪", main:"🏭", superadmin:"🏭" };
const ST_CFG = {
...
// LOGIN
// ════════════════════════════════════════════════════════════
function LoginPage({ onLogin, theme, setTheme }:{ onLogin:(u:UserInfo)=>void; theme:ThemeMode; setTheme:(t:ThemeMode)=>void }) {
  ...
  return (
    <div className={`${theme} theme-shell`} style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",background:"var(--app-bg)",fontFamily:"var(--font-ui)"}}>
      ...
      <div style={{width:"100%",maxWidth:400}}>
        ...
        {/* Card */}
        <div style={{background:"var(--app-panel)",border:"1px solid var(--app-border)",borderRadius:24,padding:"28px 24px",boxShadow:"0 24px 64px rgba(0,0,0,.35)"}}>
          <div className="form-group">
            <label className="form-label">Foydalanuvchi</label>
            <input className="crm-input" value={userId} onChange={e=>setUserId(e.target.value)} placeholder="user id" />
          </div>
          <div className="form-group">
          ...
          {error && (
            <div style={{background:"rgba(248,81,73,.1)",border:"1px solid rgba(248,81,73,.3)",borderRadius:10,padding:"10px 14px",color:"#f85149",fontSize:12,fontWeight:700,marginBottom:14}}>❌ {error}</div>
          )}

          <button className="btn-primary" onClick={handleLogin} disabled={loading} style={{width:"100%",marginBottom:20,padding:"13px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {demos.map(d=>(
              <button key={d.id} onClick={()=>{setUserId(d.id);setPassword(d.password);setError("");}}
                style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",borderRadius:12,border:`1.5px solid ${userId===d.id?d.color:"var(--app-border)"}`,background:userId===d.id?`${d.color}18`:"var(--app-panel-soft)",cursor:"pointer",color:"var(--app-text)",fontSize:12,fontWeight:700,fontFamily:"inherit",transition:"all .15s"}}>
        ...
        <div style={{textAlign:"center",marginTop:16}}>
          <button onClick={()=>setTheme(theme==="dark"?"light":"dark")} style={{background:"none",border:"none",color:"var(--app-muted)",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit"}}>
...
export default function CRMApp() {
  ...
  return (
    <div className={`${theme} theme-shell`} style={{display:"flex",height:"100vh",background:"var(--app-bg)",fontFamily:"var(--font-ui)",color:"var(--app-text)",overflow:"hidden"}}>
      ...
      {/* TOAST */}
      {toast && (
        <div style={{position:"fixed",top:20,right:20,zIndex:9999,padding:"13px 20px",borderRadius:14,fontWeight:800,fontSize:13,boxShadow:"0 12px 32px rgba(0,0,0,.4)",animation:"toastIn .3s cubic-bezier(.22,.68,0,1.2)",display:"flex",alignItems:"center",gap:8,maxWidth:340,background:toast.type==="success"?"#3fb950":"#f85149",color:"#fff"}}>
...
function PageWrap({ title, sub, action, children }:{ title?:any; sub?:any; action?:any; children:any }) {
  return (
    <div className="page-pad" style={{padding:"28px 24px"}}>
      {(title||action) && (
        <div className="action-row" style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12,marginBottom:26}}>
          <div>
            {title && <div style={{fontSize:22,fontWeight:900,letterSpacing:-.4,marginBottom:4}}>{title}</div>}
...
// DASHBOARD
// ════════════════════════════════════════════════════════════
function DashboardTab({ reports, user, setTab, transfers, orders, companies, t }:any) {
  if (!reports) return <PageWrap><div style={{color:"var(--app-muted)",padding:40,textAlign:"center"}}>{t.loading}</div></PageWrap>;
  ...
  return (
    <PageWrap title={`${user.branchIcon} ${user.branchName}`} sub={today}
      ...
      <div className="stat-row" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:28}}>
        {stats.map((s,i)=>(
          <div key={i} className="stat-card fade-up" style={{animationDelay:`${i*60}ms`,borderTop:`3px solid ${s.c}`,background:`linear-gradient(160deg,${s.bg},var(--app-panel))`}}>
            <div style={{fontSize:28,marginBottom:10}}>{s.i}</div>
            <div style={{color:"var(--app-muted)",fontSize:11,fontWeight:700,marginBottom:6,letterSpacing:.3}}>{s.l}</div>
            <div style={{fontWeight:900,fontSize:s.v.length>10?16:20,color:s.c,letterSpacing:-.3}}>{s.v}</div>
          </div>
        ))}
      </div>
              ...
              <button key={b.branch} className="fade-up branch-summary-card" onClick={()=>b.branch==="shop"&&setTab("shop-sales")}
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {[["Sklad",fmtM(b.stockValue),"#3fb950"],["Kam qoldi",b.lowStockCount,b.lowStockCount>0?"#f85149":"#3fb950"]].map(([l,v,c])=>(
                    <div key={String(l)} style={{background:"var(--app-panel-soft)",borderRadius:10,padding:"9px 11px"}}>
                      <div style={{fontSize:10,color:"var(--app-muted)",marginBottom:4,fontWeight:700}}>{l}</div>
                      <div style={{fontWeight:900,color:String(c),fontSize:13}}>{v}</div>
                    </div>
                  ))}
                </div>
              </button>
      ...
      <div className="table-wrap">
        <table className="crm-table">
          <thead><tr><th>ID</th><th>Filial</th><th>Qiymat</th><th>Status</th><th className="hide-mobile">Sana</th></tr></thead>
          <tbody>
            {transfers.slice(0,6).map((t:any)=>{
              ...
              return (
                <tr key={t.id}>
                  ...
                  <td className="hide-mobile" style={{fontSize:11,color:"var(--app-muted)"}}>{fmtD(t.createdAt)}</td>
                </tr>
              );
            })}
            {transfers.length===0&&<tr><td colSpan={5} style={{textAlign:"center",color:"var(--app-muted)",padding:32}}>Transfer yo'q</td></tr>}
          </tbody>
        </table>
      </div>
    </PageWrap>
...
function WarehouseTab({ products, stock, user, fetchAll, showToast, t }:any) {
  ...
  const qtyColor = (qty:number, min:number) => qty<=min?"#f85149":qty<=min*2?"#f0a500":"#3fb950";

  return (
    <PageWrap title="📦 Sklad"
      sub={<>Jami: <strong style={{color:"#3fb950"}}>{fmtM(totalVal)}</strong> {lowStock>0&&<span style={{color:"#f85149",marginLeft:8}}>⚠️ {lowStock} ta kam</span>}</>}>

      {editP && (
        <Modal onClose={()=>setEditP(null)}>
          <div className="modal-title">{t.editStock}</div>
          <div style={{background:"var(--app-panel-soft)",borderRadius:12,padding:14,marginBottom:18,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontWeight:800,fontSize:15}}>{editP.name}</div>
              <div style={{fontSize:12,color:"var(--app-muted)",marginTop:2}}>Hozirgi: <strong style={{color:"#f0a500"}}>{fmt(stock[editP.id]||0)} {editP.unit}</strong></div>
            </div>
            <div style={{fontSize:32}}>{(stock[editP.id]||0)<=editP.minStock?"🔴":"🟢"}</div>
          </div>
          <div className="form-group">
            <label className="form-label">YANGI MIQDOR ({editP.unit})</label>
            <input className="crm-input" type="number" value={newQty} onChange={e=>setNewQty(e.target.value)} onKeyDown={e=>e.key==="Enter"&&saveStock()} autoFocus />
          </div>
      ...
      <div style={{marginBottom:16,maxWidth:320}}>
        <input className="crm-input" placeholder={`🔍 ${t.searchProduct}`} value={search} onChange={e=>setSearch(e.target.value)} />
      </div>

      <div className="table-wrap">
        <table className="crm-table">
          <thead><tr><th>#</th><th>Mahsulot</th><th className="hide-mobile">Kategoriya</th><th>Birlik</th><th>Sklad</th><th>Holat</th>{isSA&&<th>Amal</th>}</tr></thead>
          <tbody>
            {filtered.map((p:Product,i:number)=>{
              const qty=stock[p.id]||0; const c=qtyColor(qty,p.minStock);
            ...
            {filtered.length===0&&<tr><td colSpan={7} style={{textAlign:"center",color:"var(--app-muted)",padding:40}}>Mahsulot topilmadi</td></tr>}
          </tbody>
        </table>
      </div>
    </PageWrap>
...
// TRANSFERLAR
// ════════════════════════════════════════════════════════════
function TransfersTab({ transfers, products, user, fetchAll, showToast, t }:any) {
    ...
    <PageWrap title="🔄 Transferlar"
        ...
        <Modal onClose={()=>setShowModal(false)}>
          ...
          <div className="form-group">
            ...
            <select className="crm-input" value={form.toBranch} onChange={e=>setForm({...form,toBranch:e.target.value})}>
              {Object.entries(BNAME).filter(([k])=>k!=="main").map(([k,v])=><option key={k} value={k}>{BICON[k]} {v}</option>)}
            </select>
          </div>
          <div className="form-group">
      ...
      {detail && (
        <Modal onClose={()=>setDetail(null)}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:18}}>
            ...
            <span className="badge" style={{background:ST_CFG[detail.status as keyof typeof ST_CFG].bg,color:ST_CFG[detail.status as keyof typeof ST_CFG].c}}>{ST_CFG[detail.status as keyof typeof ST_CFG].i} {ST_CFG[detail.status as keyof typeof ST_CFG].l}</span>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
            {[["Filial",`${BICON[detail.toBranch]} ${BNAME[detail.toBranch]}`],["So'ragan",detail.requestedBy],["Sana",fmtD(detail.createdAt)],["Tasdiqlagan",detail.approvedBy||"—"]].map(([l,v])=>(
      ...
      {groups.pending.length>0 && (
        <div style={{marginBottom:24}}>
          <div style={{fontSize:13,fontWeight:800,color:"#f0a500",marginBottom:12,display:"flex",alignItems:"center",gap:6}}>⏳ Kutilayotgan <span style={{background:"rgba(240,165,0,.15)",padding:"2px 8px",borderRadius:12}}>{groups.pending.length}</span></div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {groups.pending.map((t:any)=><TransferCard key={t.id} t={t} isSA={isSA} onDetail={()=>setDetail(t)} onApprove={()=>approve(t.id)} onReject={()=>reject(t.id)} />)}
          </div>
        </div>
      )}

      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {groups.other.map((t:any)=><TransferCard key={t.id} t={t} isSA={isSA} onDetail={()=>setDetail(t)} onApprove={()=>approve(t.id)} onReject={()=>reject(t.id)} />)}
        {transfers.length===0&&<div style={{textAlign:"center",padding:60,color:"var(--app-muted)"}}>Transfer yo'q</div>}
      </div>
    </PageWrap>
  );
}
    ...
    <div style={{background:"var(--app-panel)",border:"1px solid var(--app-border)",borderRadius:14,padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,transition:"all .2s"}}
      ...
      <div>
        <div style={{fontWeight:800,marginBottom:4,display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontFamily:"monospace",fontSize:10,color:"var(--app-muted)",background:"var(--app-panel-soft)",padding:"2px 7px",borderRadius:6}}>{t.id.slice(-8)}</span>
        </div>
...
function OrdersTab({ orders, products, companies, fetchAll, showToast, t }:any) {
  ...
  return (
    <PageWrap title="🛒 Orderlar" sub={`${orders.length} ta order`}
      ...
      {showModal && (
        <Modal onClose={()=>setShowModal(false)}>
                ...
                return (
                  <div key={i} style={{background:"var(--app-panel-soft)",borderRadius:12,padding:12,marginBottom:10}}>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 80px 110px 36px",gap:8,marginBottom:prod?8:0}}>
                      <select className="crm-input" value={item.pid} onChange={e=>{const n=[...items];const p=products.find((x:Product)=>x.id===e.target.value);n[i].pid=e.target.value;n[i].price=p?.pricePerUnit||0;setItems(n);}}>
                    ...
                    {prod&&item.price>0&&<div style={{fontSize:12,color:"var(--app-muted)"}}>{prod.name} × {item.qty} = <strong style={{color:"#3fb950"}}>{fmtM(item.qty*item.price)}</strong></div>}
                  </div>
                );
              })}
          ...
          <div style={{display:"flex",gap:10}}>
            ...
            <button className="btn-primary" onClick={submit} disabled={loading||!form.companyId} style={{flex:2}}>{loading?`${t.sending}`:`💾 ${t.save} + ${t.savedToStock}`}</button>
          </div>
        </Modal>
      )}
      ...
      <div className="table-wrap">
        <table className="crm-table">
          <thead><tr><th>Order ID</th><th>Firma</th><th className="hide-mobile">Mahsulotlar</th><th>Jami</th><th>To'lov</th><th className="hide-mobile">Sana</th></tr></thead>
          <tbody>
            {orders.map((o:Order)=>{
            ...
            {orders.length===0&&<tr><td colSpan={6} style={{textAlign:"center",color:"var(--app-muted)",padding:48}}>
              <div style={{fontSize:36,marginBottom:8}}>🛒</div>
              <div style={{fontWeight:700}}>Order yo'q</div>
              <div style={{fontSize:12,marginTop:4}}>"+ Yangi order" tugmasini bosing</div>
            </td></tr>}
          </tbody>
        </table>
      </div>
...
// MAHSULOTLAR
// ════════════════════════════════════════════════════════════
function ProductsTab({ products, stock, companies, fetchAll, showToast, t }:any) {
  const [showModal, setShowModal] = useState(false);
  const [search, setSearch] = useState("");
  ...
  return (
    <PageWrap title="🏷️ Mahsulotlar" sub={`${products.length} ta mahsulot`}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:4}}>
            ...
            <div className="form-group">
              ...
              <select className="crm-input" value={form.category} onChange={e=>setForm({...form,category:e.target.value})}>
                {["gosht","sabzavot","don","sut","meva","ziravorlar","ichimlik","boshqa"].map(c=><option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div className="form-group">
              ...
              <select className="crm-input" value={form.unit} onChange={e=>setForm({...form,unit:e.target.value})}>
                {["kg","g","l","ml","dona","qop","quti","karobka"].map(u=><option key={u} value={u}>{u}</option>)}
              </select>
            </div>
            <div className="form-group">
      ...
      <div className="table-wrap">
        <table className="crm-table">
          <thead><tr><th>#</th><th>Nomi</th><th className="hide-mobile">Kategoriya</th><th>Birlik</th><th className="hide-mobile">QR / Shtrix</th><th>Narxi</th><th>Sklad</th></tr></thead>
          <tbody>
            {filtered.map((p:Product,i:number)=>{
            ...
            {filtered.length===0&&<tr><td colSpan={7} style={{textAlign:"center",color:"var(--app-muted)",padding:48}}>
...
// FIRMALAR
// ════════════════════════════════════════════════════════════
function FirmsTab({ companies, orders, companyPayments, fetchAll, showToast, t }:any) {
  ...
  const cOrders  = (id:string) => orders.filter((o:Order)=>o.companyId===id);
  const cDebt    = (id:string) => cOrders(id).reduce((s:number,o:Order)=>s+(o.totalPrice-o.paidAmount),0);
  const cTotal   = (id:string) => cOrders(id).reduce((s:number,o:Order)=>s+o.totalPrice,0);
    ...
    <PageWrap title="🏢 Firmalar" sub={`${companies.length} ta firma`}
          ...
          {view==="pay" && (
            <div>
              ...
              <div style={{textAlign:"center",borderRadius:16,padding:"24px 20px",marginBottom:20,background:cDebt(selected.id)>0?"linear-gradient(135deg,rgba(248,81,73,.08),transparent)":"linear-gradient(135deg,rgba(63,185,80,.08),transparent)",border:`1px solid ${cDebt(selected.id)>0?"rgba(248,81,73,.2)":"rgba(63,185,80,.2)"}`}}>
                <div style={{fontSize:12,color:"var(--app-muted)",marginBottom:8,fontWeight:700,letterSpacing:.5}}>UMUMIY QARZ</div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {cOrders(selected.id).map((o:Order)=>{
                  const debt=o.totalPrice-o.paidAmount; const pay=PAY_CFG[o.payStatus];
                  return (
      ...
      <div className="firm-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
        {companies.map((c:Company,i:number)=>{
          const debt=cDebt(c.id); const orderCnt=cOrders(c.id).length;
          return (
